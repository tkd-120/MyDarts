...
function getCricketTargetKey(type, number) {
  if (type === 'OB' || type === 'IB') return 'BULL'
  if (number && CRICKET_TARGETS.includes(number)) return number
  return null
}

function getMarksForThrow(type) {
  if (type === 'MISS') return 0
  if (type === 'OB') return 1
  if (type === 'IB') return 2
  if (type === 'D') return 2
...
function isAllCricketTargetsClosed(player) {
  return CRICKET_TARGETS.every((target) => player.marks[target] >= 3)
}

function isCricketLeader(candidate) {
  return game.players.every((player) => player.id === candidate.id || candidate.cricketScore >= player.cricketScore)
}
...
function recordThrow({ type, number }) {
  ...
  if (game.mode === GAME_MODES.ZERO_ONE_301) {
    return handleZeroOneThrow({ type, number, player, round })
  }

  if (game.mode === GAME_MODES.CRICKET) {
    return handleCricketThrow({ type, number, player, round })
  }
...
function handleZeroOneThrow({ type, number, player, round }) {
  if (round.startRemaining == null) {
...
function handleCricketThrow({ type, number, player, round }) {
  const targetKey = getCricketTargetKey(type, number)
  const marks = targetKey ? getMarksForThrow(type) : 0

  if (!targetKey) {
    round.throws.push({ type, number: number ?? null, score: 0, marks })
    round.roundScore = round.throws.reduce((sum, item) => sum + item.score, 0)
    recalcTotals()
    if (round.throws.length === THROWS_PER_ROUND) advanceTurn()
    return
  }

  const previousMarks = player.marks[targetKey]
  const afterMarks = previousMarks + marks
  const overflowBefore = Math.max(0, previousMarks - 3)
  const overflowAfter = Math.max(0, afterMarks - 3)
  const newlyOverflow = Math.max(0, overflowAfter - overflowBefore)
  const anyOpponentOpen = game.players.some((p) => p.id !== player.id && p.marks[targetKey] < 3)

  const scoreGain = anyOpponentOpen ? newlyOverflow * (targetKey === 'BULL' ? 25 : Number(targetKey)) : 0
  player.marks[targetKey] = afterMarks
  player.cricketScore += scoreGain

  round.throws.push({ type, number: number ?? null, score: scoreGain, marks })
  round.roundScore = round.throws.reduce((sum, item) => sum + item.score, 0)
  recalcTotals()

  if (isAllCricketTargetsClosed(player) && isCricketLeader(player)) {
    assignWinners([player.id])
    game.status = 'finished'
    return
  }
...
function assignWinners(ids) {
  game.winnerIds = ids
}

function determineWinnersAfterRounds() {
  ...
  if (game.mode === GAME_MODES.COUNT_UP) {
    ...
    assignWinners(game.players.filter((p) => p.totalScore === topScore).map((p) => p.id))
    return
  }

  if (game.mode === GAME_MODES.ZERO_ONE_301) {
    const bestRemaining = Math.min(...game.players.map((p) => p.remainingScore))
    assignWinners(game.players.filter((p) => p.remainingScore === bestRemaining).map((p) => p.id))
    return
  }

  const closedPlayers = game.players.filter((player) => isAllCricketTargetsClosed(player))
  const candidates = closedPlayers.length ? closedPlayers : game.players
  const bestScore = Math.max(...candidates.map((p) => p.cricketScore))
  assignWinners(candidates.filter((p) => p.cricketScore === bestScore).map((p) => p.id))
}

function advanceTurn() {
  if (game.currentRoundIndex === game.maxRounds - 1 && game.currentPlayerIndex === game.players.length - 1) {
    determineWinnersAfterRounds()
    game.status = 'finished'
    return
  }
...
const roundScoreLabel = computed(() => {
  if (game.mode === GAME_MODES.COUNT_UP) return 'ラウンド小計'
...
function scoreboardValue(player) {
  if (game.mode === GAME_MODES.COUNT_UP) return player.totalScore
  if (game.mode === GAME_MODES.ZERO_ONE_301) return player.remainingScore
  return player.cricketScore
}

function resultValue(player) {
  if (game.mode === GAME_MODES.COUNT_UP) return player.totalScore
  if (game.mode === GAME_MODES.ZERO_ONE_301) return player.remainingScore
  return player.cricketScore
}
...
function formatThrow(throwItem) {
  if (!throwItem) return '-'
  const base = `${throwItem.type}${throwItem.number ? ` ${throwItem.number}` : ''}`
  if (game.mode === GAME_MODES.CRICKET) {
    const marksLabel = throwItem.marks != null ? `${throwItem.marks}M` : ''
    const scoreLabel = throwItem.score ? `+${throwItem.score}` : ''
    const meta = [marksLabel, scoreLabel].filter(Boolean).join(' / ')
    return meta ? `${base} (${meta})` : base
  }
  return `${base} = ${throwItem.score}`
}
  ...
  <div class="app-shell">
    <header class="app-header">
      <div>
        ...
        <h1>MyDarts</h1>
        <p class="subtitle">スマホで遊べるマルチモードスコア計算</p>
      </div>
          ...
          <div v-for="slot in THROWS_PER_ROUND" :key="slot" class="throw-card">
            ...
            <p class="value">
              <template v-if="currentRoundDisplayThrows[slot - 1]">
                {{ formatThrow(currentRoundDisplayThrows[slot - 1]) }}
              </template>
    ...
    <section v-else-if="isFinished" class="panel">
      ...
      <ul class="result-list">
        <li v-for="player in resultRows" :key="player.id" class="result-item" :class="{ winner: game.winnerIds.includes(player.id) }">
          <span class="name">{{ player.name }}</span>
          <span class="score">{{ resultValue(player) }}</span>
        </li>
      </ul>
      <div class="actions">
        <button class="ghost" type="button" :disabled="!hasHistory" @click="undoThrow">UNDO</button>
