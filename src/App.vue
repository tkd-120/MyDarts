...
const inputMethod = ref('dartboard')
...
function handleBoardHit(payload) {
...
<template>
  <div class="app-shell">
    ...
    <section v-else-if="isPlaying" class="panel">
      ...
      <div class="controls">
        <div class="control-row control-top">
          <p class="label">入力</p>
          <div class="pill-group">
            <button
              v-for="mode in ['dartboard', 'keypad']"
              :key="mode"
              type="button"
              :class="['pill', { active: inputMethod === mode }]"
              @click="inputMethod = mode"
            >
              {{ mode === 'dartboard' ? 'ダーツ盤' : 'ボタン' }}
            </button>
          </div>
          <button class="ghost" type="button" :disabled="!hasHistory" @click="undoThrow">UNDO</button>
        </div>

        <div v-if="inputMethod === 'dartboard'" class="dartboard-panel">
          <DartboardInput @hit="handleBoardHit" />
        </div>
